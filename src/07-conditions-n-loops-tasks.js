...
function isCreditCardNumber(ccn) {
  const ccnString = ccn.toString().split('').map(Number);
  let sum = 0;
  let isSec = false;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const current = str[i];
    if (bracketsOpen.includes(current)) {
      stack.push(current);
    } else {
      if (stack.length === 0) {
        return false;
...
function getCommonDirectoryPath(pathes) {
  ...
  const result = rotate(splitStrings(pathes, '/')).filter(allElementsEqual).map(elAt(0)).join('/');
  if (result.length === 0) {
    if (pathes.every((s) => s[0] === '/')) return '/';
    return '';
  }
  return `${result}/`;
}
...
function evaluateTicTacToePosition(position) {
  ...
  position.map((row) => {
    while (row.length < 3) row.push(undefined);
    return 0;
  });

  const field = position.flat();

  let winner;
  winVectors.forEach((vector) => {
    const a = field[vector[0]];
    const b = field[vector[1]];
    const c = field[vector[2]];
